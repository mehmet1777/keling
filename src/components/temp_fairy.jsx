            ...
            <div className="relative">
              ...
              >
                <img 
                  ...
                  style={{ filter: 'drop-shadow(0 0 15px rgba(255, 158, 249, 0.9))' }}
                />
              ...
              <div className="absolute top-0 left-full ml-6 bg-gradient-to-r from-purple-600 to-pink-600 p-4 rounded-lg shadow-lg">
                <h3 className="text-white font-bold mb-2 text-center">Bütün Harfler</h3>
                <div className="flex flex-wrap gap-2 justify-center max-w-xs">
                  {level.words.map((word, wordIndex) => (
                    <div key={`fairy-word-${wordIndex}`} className="mb-2 w-full">
                      <div className="text-xs text-white mb-1">{word.english}</div>
                      <div className="flex gap-1 justify-center">
                        {word.turkish.split('').map((letter, letterIndex) => (
                          <motion.div 
                            key={`fairy-letter-${wordIndex}-${letterIndex}`}
                            className="w-8 h-8 bg-white/40 rounded-md flex items-center justify-center text-white font-bold shadow-lg"
